...
The CSV includes `f0_std_hz`, HNR, HF ratios, and voiced ratio. It does not include CPPS. For v7 calibration, CPPS-style signal, spectral flatness, spectral centroid, ZCR, and app-aligned HNR/F0/HF features were re-extracted from `analysis_wav_path` at 16 kHz to match `App.js` more closely.
...
- Deployment form: scaler means/stds, weight vector, intercept, and two tri-state decision thresholds embedded in `App.js`
