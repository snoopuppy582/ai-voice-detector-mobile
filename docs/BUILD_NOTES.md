...
The current workspace refactors the same v7 SVM source into `src/` modules after the production v8 AAB listed below was built. Use the v8 AAB only if you intentionally submit that previously built artifact. For exact source-to-artifact traceability from this folder, create a new production AAB and update this file.

Post-refactor source verification on 2026-06-24:

- `node --check App.js`
- `node --check src/config/audio.js`
- `node --check src/utils/math.js`
- `node --check src/model/linearSvmModel.js`
- `node --check src/audio/analysis.js`
- `npm run doctor`
- `npx expo export --platform android --output-dir dist-android`

No post-refactor production AAB has been generated yet.

Attempted post-refactor EAS production build on 2026-06-24:

- Command: `npx eas-cli build -p android --profile production --non-interactive --wait --message "Production v9 current source SVM refactor"`
- EAS bumped the proposed build from Android versionCode `8` to `9`.
...
- v8 release note: includes the v7 linear soft-margin SVM calibration; EAS auto-incremented Play build version from 7 to 8.
...
Upload this AAB to Google Play closed testing only if you are intentionally submitting the previously built v8 artifact. Rebuild first if submitting the current refactored source.
...
Completed for the pre-refactor v7/v8 SVM source baseline:
